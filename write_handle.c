	...
	if (y > 1)
	{
		...
		if (f & F_MINUS)
			return (write(1, &buffer[0], 1) +
					write(1, &buffer[BUFF_SIZE - i - 1], y - 1));
		else
			return (write(1, &buffer[BUFF_SIZE - i - 1], y - 1) +
					write(1, &buffer[0], 1));
	}
	...
	int f, int y, int pre, int s)
{
	...
	return (write_num(ind, buffer, f, y, pre,
...
int write_num(int ind, char buffer[],
	int f, int y, int prec,
	int length, char padd, char extra_c)
{
	int i, padd_start = 1;

	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && y == 0)
...
int write_pointer(char buffer[], int ind, int length,
	int y, int f, char padd, char extra_c, int padd_start)
{
	int i;

	if (y > length)
	{
		for (i = 3; i < y - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (f & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
